with QUESTION_SOLVED as (
	...
	and question.id_community in (select id from community where name = 'chemistry.stackexchange.com')
),
 BEST_ANSWER as (
	select  
		  answer.*
...
ALL_ANSWERS as (
	...
	from post answer
	where answer.post_type = 2
	and answer.id_community  in (select id from community where name = 'chemistry.stackexchange.com')
