 ...
 PARTICIPATION_LAYER as (
	with QUESTION_TAGS as (
		select
				...
				tag2 ,
				tag3
			from post_tags('chemistry.stackexchange.com')
	), USER_BY_DISCUSSION as (
		select  id_question, id_user from comm_user_by_discussion('chemistry.stackexchange.com')
	),
	LAYERS as (
		-- recupera as camadas de cada tag
		select * from tag_layer('chemistry.stackexchange.com')
	),
	USER_TAGS as (
...
USER_ENTROPY as (
	select * from user_entropy('chemistry.stackexchange.com')
)
